...
var notify_array=["敏感族群應減少在戶外劇烈活動","所有族群應減少在戶外活動","對所有族群非常不健康應減少戶外活動","對所有族群應減少戶外活動"]
...
function generator(input_data){
    ...
    if(sorted_list.length===1){
        str= "空氣品質普遍為"+segment_array[sorted_list[0]];
        if (sorted_list[0]>1){
        ...
        if(sorted_list.length===2){
            ...
            var sort_state=[sorted_list[0],sorted_list[sorted_list.length-2]].sort(sortPriority);
            str= "空氣品質多為"+segment_array[sort_state[0]]+"至"+segment_array[sort_state[1]]
            if (sort_state[1]>1){
                str+="，"+notify_array[sort_state[1]-2]+"。"
            }
        }
